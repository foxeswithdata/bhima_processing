...
forest_cells <- read.csv("data/plantFATE_rerun_data/simulation_list.csv") %>%
  filter(biodiversity == "lb" & new_forest == TRUE)


species_data_out_all <- lapply(1:nrow(forest_cells), function(i){
  results_directory = paste("out/PlantFATE_reruns_out/individual_cell_simulations/GEB_step4_lb_af_10/cell_", 
                            forest_cells$cell[i],
                            "_lb/", 
                            sep = "")
  species_data_out = read.csv(paste(results_directory, "/Y_PFATE.csv", sep = ""))
  species_data_out$biodiversity <- "Low"
...
filename_figure = "figure_a11_total_biomass_new_forest_lb"
