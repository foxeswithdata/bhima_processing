...
plantfate_input_folder = "data/prepare_plantfate_spinup"
input_run = plantfate_input_folder
input_run = "data/ssp3_out/GEB_step4_hb_af_10/hydrology.soil/"
output_folder = Path("out/environmental_data_processed/GEB_step4_hb_af_10/")
...
da_temp = xr.open_zarr(
            str(input_run + "temperature_K.zarr"),
            consolidated=False
        )
da_vpd = xr.open_zarr(
        str(input_run + "vapour_pressure_deficit_kPa.zarr"),
            consolidated = False
        )
da_ppfd = xr.open_zarr(
            str(input_run + "photosynthetic_photon_flux_density_W_m2.zarr"),
            consolidated = False
        )
da_swp = xr.open_zarr(
            str(input_run + "soil_water_potential_MPa.zarr"),
            consolidated=False
        )
da_nr = xr.open_zarr(
            str(input_run + "net_absorbed_radiation_vegetation_MJ_m2_day.zarr"),
            consolidated=False
        )
...
outpath = Path(output_folder, "out_data_monthly.csv")
print(outpath)
...
outpath = Path(output_folder, "out_data_yearly.csv")
print(outpath)
da_merge.to_csv(outpath, index = True)
...
outpath = Path(output_folder, "out_data_daily.csv")
print(outpath)
da_merge.to_csv(outpath, index = True)
